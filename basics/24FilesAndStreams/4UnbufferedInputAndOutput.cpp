...
--------------------------------------
- The simplest memory block is a single Character, and there are member
functions for that.
...
- As we are not using a buffer managed by the stream, we have to provide our
  ...
  * For read(), the buffer must be large enough to store all the data we expect
    ...
    * The numbers of characters in the buffer
      ...
      ifile.read(filebuf, filesize); //Fetch data from the file into the memory
      buffer
      cout.write(filebuf, filesize); //Send the data from the memory buffer to
      output
---------
gcount()
---------
...
int main() {
  ...
  ifile.read(filebuf,
             filesize); // Fetch data from the file into the memory buffer
